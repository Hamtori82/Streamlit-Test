...
if choice == menu[0] :
    ...
    if work_range == '시작구간 선택 (200개 단위)':
        ...
        if len(gc3) < 200:
            gc2 = gc1.get(f'A{min_number-1}:E{len(gc3)}')
            print(gc2)
            gc3 = pd.DataFrame(gc2, columns=gc_df[0])
        else:
            try:
        ...
        max_number = st.number_input(
		f'숫자를 입력하세요({min_number}-{len(gc3)+1})',
        min_value=min_number, max_value=len(gc3)+1, value=min_number+1, step=1)
    ...
    while row < gc3.shape[0]:
        ...
        if text == None:
            ...
            if selected_words:
                ...
                gc3.iloc[row, token_col] = token_value
                ## google sheet 바로 적용!
                gc1.update_acell(f'F{int(gc3.iloc[row,0])+2}', token_value)
                #if st.button('Google Sheet 적용'):
                #    gc1.update_acell(f'F{num+2}', token_value)
