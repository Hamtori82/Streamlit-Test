...
def data_update(sheet_name, token_value, num):
    ...
    if __name__ == '__main__':
        ...
        with SSHTunnelForwarder(
            ...
            ## 데이터 프레임의 values를 리스트로 변환하기
            
            try:
                with connection.cursor() as cursor:
                    cursor.execute(f" UPDATE {sheet_name} SET tokens_positive_no='{token_value.strip()}' WHERE id={num}")
                    connection.commit()
            finally:
                # 연결 종료
                connection.close()
...
if choice == menu[0] :
    ...
    ## db 데이터 불러오기
    df = pd.DataFrame(db_select(sheet_name), columns=['id','folder_name', 'image_name', 'result_text', 'convert_text','tokens_positive_no'])
            ...
            if selected_words:
                ...
                if method == '수동':
                    if st.button('입력',key = 'n' + str(count2)):
                        data_update(sheet_name, token_value, num)
...
## get_all_values() 함수를 이용해 코드를 실행하면 리스트 형태의 값이 리턴됩니다.
            ...
            if text is not None:
                ...
                if selected_words:
                    ...
                    if st.button('수정',key = 'n' + str(row)):
                        data_update(sheet_name, token_value2, row+min_number)
    ...
    st.write('---------------------')
    st.write('실제 데이터')
    df2 = pd.DataFrame(db_select(sheet_name), columns=['id','folder_name', 'image_name', 'result_text', 'convert_text','tokens_positive_no'])
    st.dataframe(df2[:row])
